...
router.get('/global', auth_1.optionalAuth, async (req, res) => {
    try {
        ...
        const leaderboard = users.map((user, index) => ({
            rank: parseInt(offset, 10) + index + 1,
            ...user,
        }));
        // Get current user's rank if authenticated
        let userRank = null;
        if (req.user) {
            const higherRankedCount = await prisma_1.default.user.count({
                where: {
                    totalXp: {
                        gt: (await prisma_1.default.user.findUnique({
...
router.get('/weekly', auth_1.optionalAuth, async (req, res) => {
    try {
        ...
        const weeklyXp = await prisma_1.default.userAnswer.groupBy({
            by: ['userId'],
            where: {
        ...
        let userWeeklyXp = 0;
        if (req.user) {
            const userWeeklyData = await prisma_1.default.userAnswer.aggregate({
                where: {
                    userId: req.user.id,
                    answeredAt: { gte: startOfWeek },
                },
            ...
            if (userWeeklyXp > 0) {
                const higherRankedCount = await prisma_1.default.userAnswer.groupBy({
                    by: ['userId'],
                    where: {
                        answeredAt: { gte: startOfWeek },
                    },
...
router.get('/category/:slug', auth_1.optionalAuth, async (req, res) => {
    try {
        ...
        const progress = await prisma_1.default.userProgress.findMany({
            ...
            take: Math.min(parseInt(limit, 10), 25),
            skip: parseInt(offset, 10),
            include: {
                user: {
                    select: {
        ...
        let userRank = null;
        if (req.user) {
            const userProgress = await prisma_1.default.userProgress.findUnique({
                where: {
                    userId_categoryId: {
                        userId: req.user.id,
            ...
            if (userProgress) {
                const higherRankedCount = await prisma_1.default.userProgress.count({
                    where: {
                        categoryId: category.id,
